...
int changeDir(char dirName[])
{
    ...
    //修改当前目录
    dirBlock = currentDirTable->dirs[unitIndex].startBlock;
...
int creatDir(char dirName[])
{
    ...
    if(addDirUnit(currentDirTable, dirName, 0, dirBlock) == -1)
        return -1;
    //为新建的目录添加一个到父目录的目录项
    newTable = (dirTable*)getBlockAddr(dirBlock);
...
int addDirUnit(dirTable* myDirTable, char fileName[], int type, int FCBBlockNum)
{
    ...
    //构建新目录项
    newDirUnit = &myDirTable->dirs[dirUnitAmount];
...
int deleteFile(char fileName[])
{
    ...
    //查找文件的目录项位置
    unitIndex = findUnitInTable(currentDirTable, fileName);
    if(unitIndex == -1)
    {
        printf("file not found\n");
        return -1;
    }
    myUnit = currentDirTable->dirs[unitIndex];
    //判断类型
    if(myUnit.type == 0)//目录
    {
        printf("not a file\n");
...
int deleteDir(char dirName[])
{
    ...
    //查找文件
    unitIndex = findUnitInTable(currentDirTable, dirName);
    if(unitIndex == -1)
    {
        printf("file not found\n");
        return -1;
...
FCB* my_open(char fileName[])
{
    ...
    //控制块
    FCBBlock = currentDirTable->dirs[unitIndex].startBlock;
    myFCB = (FCB*)getBlockAddr(FCBBlock);
    return myFCB;
}
...
{
	char* data;
    int unitIndex, FCBBlock, i, dataSize;
	FCB* myFCB;
	unitIndex = findUnitInTable(currentDirTable, fileName);
    ...
    //读数据
    data = (char*)getBlockAddr(myFCB->blockNum);
    /* 获取记录读者数量的锁 */
    /* 根据拥有锁的进程数量来判断是否是第一个读者 */
    /* 如果是第一个读者就负责锁上写者锁 */

    dataSize = myFCB->dataSize;
...
{
    int unitIndex = findUnitInTable(currentDirTable, fileName);
	int i, FCBBlock, contentLen, fileSize;
	FCB* myFCB;
	char* data;
    if(unitIndex == -1)
    {
    ...
    fileSize = myFCB->fileSize * block_szie;
    data = (char*)getBlockAddr(myFCB->blockNum);
    /* 获得写者锁 */
    //在不超出文件的大小的范围内写入
